...
Parameter <- R6::R6Class(
    ...
    active = list(
        ...
        path = function(value) {
            if (missing(value)) return(self$data$Path)
            self$data$Path <- value
        },

        #' @field value The value of the parameter
        value = function(value) {
            if (missing(value)) return(self$data$Value)
            self$data$Value <- value
        },

        #' @field unit The unit of the parameter (if any)
        unit = function(value) {
            if (missing(value)) return(self$data$Unit)
            if (!is.null(value)) {
        ...
        value_origin = function(value) {
            if (missing(value)) return(self$data$ValueOrigin)
            if (!is.null(value)) {
