...
test_that("Individual handles missing data gracefully", {
    ...
    expect_null(minimal_individual$parameters)
...
test_that("Individual initializes parameters correctly", {
    ...
    expect_s3_class(individual$parameters[[1]], "Parameter")
    expect_s3_class(individual$parameters[[2]], "Parameter")

    # Test parameter values through direct access
    expect_equal(individual$parameters[[1]]$path, "Organism|Liver|Volume")
    expect_equal(individual$parameters[[1]]$value, 1.5)
    expect_equal(individual$parameters[[1]]$unit, "L")

    expect_equal(individual$parameters[[2]]$path, "Organism|Kidney|GFR")
    expect_equal(individual$parameters[[2]]$value, 120)
    expect_equal(individual$parameters[[2]]$unit, "ml/min")
})
