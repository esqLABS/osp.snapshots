...
- `create_expression_profile()` gains `reference_concentration`, `half_life_liver`, and `half_life_intestine` arguments (each with a paired `_unit` argument) that set the fixed global molecule-properties scalars directly, as a convenience over hand-authoring them in `parameters` (#169).
