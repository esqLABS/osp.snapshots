...
parameter tree); otherwise returns a plain \[Parameter\]. For backwards
compatibility, \`name\` is used as the path when \`path\` is not
provided, matching the historical behaviour of this factory.
