...
object with the specified min, max, and unit values. Either `min` or
`max` can be `NULL` to represent ranges with only an upper or lower
bound. `min` and `max` can also be equal to represent a single value.
...
- min:
  ...
  Numeric or `NULL`. Minimum value, can be `NULL` for ranges with only
  an upper bound.
