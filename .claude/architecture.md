...
| `Process` | `R/Process.R` | A single compound process (PK-Sim `CompoundProcess`): `internal_name`, `data_source`, optional `molecule`, `metabolite`, `species`, `parameters`, derived `category`. | Owned by `Compound`; one per entry of the raw `Processes` array. |
