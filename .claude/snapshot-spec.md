...
This document specifies how PKSim JSON snapshot files are converted into PKSim domain objects. Each section describes a snapshot type, its properties, and how each property maps into the domain model during the JSON-to-Project conversion driven by `PKSim.CLI snap -p`.

---

## Project

The top-level snapshot object. All other snapshots are nested arrays within it.
...
**Duplicate detection**: If two building blocks of the same type share a name, the second is skipped and an error is logged.
...
Created via `IRandomPopulationFactory.CreateFor(settings, cancellationToken, seed)`.
...
The factory is called with `addMoleculeParametersVariability: false` to prevent default variability (since it will come from the snapshot's advanced parameters instead).
...
**Version behavior**: In v9 or earlier, after mapping, relative expressions are normalized and default molecule parameters are loaded.

---

## Simulation

Created via `ISimulationFactory.CreateFrom(simulationSubject, compounds, modelProperties)`, then `ISimulationModelCreator.CreateModelFor(simulation)`.
...
Created via `IProtocolFactory.Create(mode, applicationType)`. Protocol type is determined by the snapshot: if `ApplicationType` is present, a `SimpleProtocol` is created; otherwise an `AdvancedProtocol` is created from `Schemas`.
...
Created by cloning a template: `IFormulationRepository.FormulationBy(formulationType)` then `ICloner.Clone(template)`.
...
Not created -- retrieves the existing `CompoundProperties` from the simulation via `simulation.CompoundPropertiesFor(name)`.
...
Created by cloning a template: `ICompoundProcessRepository.ProcessByName(internalName)` then `ICloner.Clone(template)`.
...
Note: Calculated alternatives are never exported and are not expected in the snapshot.

### PkaType

Not a standalone mapped object. Each entry directly modifies compound parameters.
...
Not created -- updates an existing `IParameter` found in the domain model's parameter tree.
...
Extends `Parameter` with a path for simulation-level parameter overrides.
...
Not created as a new object. Updates an existing `ValueOrigin` via `valueOrigin.UpdateAllFrom()`.
...
An array of calculation method name strings. Not created -- updates an existing cache.
...
Created via `IObjectBaseFactory.Create<AmountObserverBuilder>()` or `Create<ContainerObserverBuilder>()` based on the `Type` property.
...
Created via `IFormulaFactory.CreateDistributedTableFormula()`. Extends [TableFormula](#tableformula).
...
Created as `BaseGrid` or `DataColumn` depending on the `DataInfo.Origin` value. If `Origin == BaseGrid`, creates a `BaseGrid`; otherwise creates a `DataColumn` linked to the repository's base grid.
...
Created as `ExtendedProperty<T>` where T is determined by the `Type` property or inferred from the value.
...
Type is polymorphic: if `IndividualComparison` is present, creates `IndividualSimulationComparison`; otherwise creates `PopulationSimulationComparison`.
...
The classification type (ObservedData, Simulation, etc.) is determined by which project array the classification appears in, not by a property on the snapshot.

### ParameterIdentificationConfiguration

Not created -- updates the existing `Configuration` on the `ParameterIdentification` domain object.
...
Created via `IIdentificationParameterFactory.CreateFor(parameterSelections, parameterIdentification)`.
...
Created via a chart factory function (typically `new SimulationTimeProfileChart()` or `new IndividualSimulationComparison()`). The chart object is created first, then chart-level properties are updated in-place via `ChartMapper`.
...
A domain model object used directly in JSON. Updated via `UpdatePropertiesFrom()`.
...
Not created -- updates the existing `BasePopulationAnalysis` on the chart via context.
...
After fields are added, their pivot positions (`Area` and `Index`) are applied for `PopulationPivotAnalysis` types.

### PopulationAnalysisField

Created as one of several field types depending on which snapshot properties are present: `PopulationAnalysisOutputField` (default), `PopulationAnalysisParameterField`, `PopulationAnalysisPKParameterField`, `PopulationAnalysisCovariateField`, or `PopulationAnalysisGroupingField`.
...
Created as one of: `ValueMappingGroupingDefinition` (if `Mapping` present), `FixedLimitsGroupingDefinition` (if `Limits` present), or `NumberOfBinsGroupingDefinition` (default).
...
All nullable properties use **sparse serialization**: if a property equals its default value, it is serialized as `null` and omitted from the JSON output. During import, null values are replaced with defaults. This keeps snapshot files compact -- only non-default values appear in the JSON.

---

## Enums

All enums are serialized as their string name in JSON (via `StringEnumGenerationProvider`). Flags enums that combine multiple values are serialized as comma-separated strings.
...
Used in [Compound](#compound) to specify the primary binding protein in plasma.
...
Flags enum. Used in [ExpressionProfile](#expressionprofile) to define the molecule type.
...
Composite values: `Protein` = OtherProtein, Enzyme, Transporter. `Molecule` = Drug, Metabolite, Protein, Complex.

### Localization

Flags enum. Used in [ExpressionProfile](#expressionprofile) to specify where a protein is expressed in tissue.
...
Composite values: `InTissue` = Intracellular, Interstitial. `InBloodCells` = BloodCellsMembrane, BloodCellsIntracellular. `InVascularEndothelium` = VascEndosome, VascMembranePlasmaSide, VascMembraneTissueSide.

### MembraneLocation

**Legacy (v9 and below)**. Used in [ExpressionProfile](#expressionprofile) and [ExpressionContainer](#expressioncontainer). Converted to [Localization](#localization) during import.
...
**Legacy (v9 and below)**. Used in [ExpressionProfile](#expressionprofile). Converted to [Localization](#localization) during import.
...
Flags enum. Used in [ExpressionProfile](#expressionprofile) to classify the transporter mechanism.
...
Composite values: `Active` = Influx, Efflux, PgpLike, BiDirectional. `Passive` = Secretion, Elimination, Diffusion, Convection.

### TransportDirectionId

Used in [ExpressionContainer](#expressioncontainer) to specify the direction of transport in a specific organ/compartment.
...
Used in [Protocol (Simple)](#simple-protocol) to define the dosing schedule.
...
Used in [AdvancedParameter](#advancedparameter) to define the statistical distribution for population variability.
...
Used in [OutputMapping](#outputmapping), [Simulation](#simulation) axes, and [PopulationAnalysisField](#populationanalysischart) for axis scaling.
...
Flags enum. Used in [AlteredBuildingBlock](#alteredbuildingblock) to identify the type of a building block.
...
Composite values: `SimulationSubject` = Individual, Population. `Template` = all non-Simulation types.

### ValueOriginSourceId

Used in [ValueOrigin](#valueorigin) to identify where a parameter value came from.
...
Used in [ValueOrigin](#valueorigin) to describe how a parameter value was determined.
...
Used in [DataColumn](#datacolumn) (via `DataInfo`) to classify the origin of a data column.
...
Used in [DataColumn](#datacolumn) (via `DataInfo`) to classify auxiliary column types.
...
Used in [PopulationAnalysisChart](#populationanalysischart) to determine the chart type.
...
Used in [CurveChart](#curvechart) axes and curve options to assign curves to axes.
...
Used in [CurveChart](#curvechart) curve options and axes to define line rendering.
...
Used in [CurveChart](#curvechart) curve options to define data point markers.
...
Used in [CurveChart](#curvechart) curve options to define how points are connected.
...
Flags enum. Used in [PopulationAnalysisChart](#populationanalysischart) fields to control where a field appears in the analysis pivot layout.
...
Composite values: `ColumnArea` = ColorArea, SymbolArea. `All` = DataArea, FilterArea, ColumnArea, RowArea.
...
Used in [ExtendedProperty](#extendedproperty) to explicitly specify the .NET type for value parsing.
...
Used in [GroupingDefinition](#groupingdefinition) to control how bin labels are generated for number-of-bins groupings.
